...
describe("AssetLinkFactory", () => {
    describe("createAssetLink", () => {
        ...
        it("creates ResourceLink for dataset", () => {
            const name = "mpp-3.3.html";
            const namespace = VocabularyUtils.PREFIX;
            const dataset = new Resource({
                iri: namespace + name,
                label: "Dataset",
                types: [VocabularyUtils.DATASET, VocabularyUtils.RESOURCE]
            });
            const result = mountWithIntl(<MemoryRouter>{AssetLinkFactory.createAssetLink(dataset)}</MemoryRouter>);
            expect(result.find(ResourceLink).exists()).toBeTruthy();
        });
